...
if __name__ == "__main__":
    ...
    model_path = "a2c_multi_car_racing.zip"  # Cambia a la ruta de tu modelo A2C
    if not os.path.exists(model_path):
        print(f"Modelo no encontrado en {model_path}. Entrena el modelo antes de ejecutar este script.")
        exit(1)

    model = A2C.load(model_path)  # Cambié de PPO a A2C
    ...
    for replay in range(num_replays):
        ...
        while not done:
            # Predecir acción usando el modelo A2C
            action, _ = model.predict(obs, deterministic=True)  # Usamos deterministic=True para evitar ruido en la política
